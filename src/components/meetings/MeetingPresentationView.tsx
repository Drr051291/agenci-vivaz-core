...
function CompletedTasksCollapsible({ tasks }: { tasks: RecentTask[] }) {
  const [open, setOpen] = useState(false);
  const completedTasks = tasks.filter(t => t.status === 'concluido' || t.status === 'completed');
  if (completedTasks.length === 0) return null;

  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild>
        <Button variant="ghost" size="sm" className="w-full justify-between px-3 py-2 h-auto bg-green-50 dark:bg-green-950/20 border border-green-200/50 dark:border-green-800/30 hover:bg-green-100 dark:hover:bg-green-950/40">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 text-green-600" />
            <span className="text-sm font-medium text-green-700">Concluídas ({completedTasks.length})</span>
          </div>
          ...
          {recentTasks.length > 0 && (
            <Card className="transition-all">
              <CardContent className="p-6 lg:p-8">
                ...
                {/* Atividades Concluídas - Collapsible */}
                <CompletedTasksCollapsible tasks={recentTasks} />
              </CardContent>
            </Card>
          )}
        </>
