...
import { SectorDistributionData, SectorDistributionResponse, PIPELINE_ID, DateRange } from './types';
import { format } from 'date-fns';

interface UseSectorTrackingReturn {
...
interface UseSectorTrackingOptions {
  pipelineId?: number;
}

export function useSectorTracking(
  dateRange: DateRange,
  options: UseSectorTrackingOptions = {}
): UseSectorTrackingReturn {
  const { pipelineId = PIPELINE_ID } = options;
  
  const [data, setData] = useState<SectorDistributionData | null>(null);
  const [snapshotData, setSnapshotData] = useState<SectorDistributionData | null>(null);
  ...
  const fetchData = useCallback(async (force = false) => {
    ...
    try {
      const { data: responseData, error: fetchError } = await supabase.functions.invoke<SectorDistributionResponse>(
        'pipedrive-proxy',
        {
          body: {
            action: 'get_sector_distribution',
      ...
      if (!responseData?.success) {
        throw new Error(responseData?.error || 'Erro ao buscar dados de setor');
      }
      ...
      console.error('Error fetching sector distribution data:', err);
  ...
  const fetchSnapshotData = useCallback(async (force = false) => {
    ...
    try {
      const { data: responseData, error: fetchError } = await supabase.functions.invoke<SectorDistributionResponse>(
        ...
        {
          body: {
            action: 'get_sector_distribution_snapshot',
            pipeline_id: pipelineId,
            force,
          },
        }
      ...
      if (!responseData?.success) {
        throw new Error(responseData?.error || 'Erro ao buscar snapshot de setor');
      }

      setSnapshotData(responseData.data || null);
      snapshotFetchedRef.current = snapshotKey;
    } catch (err) {
      console.error('Error fetching sector distribution snapshot data:', err);
    } finally {
