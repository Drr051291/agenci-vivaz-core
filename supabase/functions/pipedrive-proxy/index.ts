...
const getTK = async (sb: any, f: boolean) => { const k = 'tk3'; if (!f) { const c = await cache(sb, k); if (c) return c }; const r = await pd('/v1/dealFields'); const fds = r?.data || []; const fn = (vs: string[]) => { for (const v of vs) { const m = fds.find((x: any) => x.name.toLowerCase().includes(v.toLowerCase())); if (m) return m.key }; return null }; const res = { campaign: fn(['Campanha']), adset: fn(['Conjunto']), creative: fn(['Anuncio', 'Anúncio']) }; await setC(sb, k, res, TTL.s); return res }
