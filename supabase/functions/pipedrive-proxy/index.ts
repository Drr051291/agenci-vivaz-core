...
const getArr = async (sb: any, pid: number, st: any[], sd: string, ed: string, f: boolean) => {
  ...
  console.log(`getArr pid=${pid} period=${sd}..${ed} total_all=${all.length} filtered=${flt.length}`)
...
const getTK = async (sb: any, f: boolean) => { const k = 'tk7'; if (!f) { const c = await cache(sb, k); if (c) return c }; const r = await pd('/v1/dealFields'); const fds = r?.data || []; const grps = await pd('/v1/dealFields', { limit: '500' }); const allF = grps?.data || []; const utmFs = allF.filter((x: any) => { const gn = typeof x.group === 'object' && x.group ? (x.group.name || '') : ''; return gn.toUpperCase().includes('UTM') }); console.log('UTM detection:', utmFs.length, utmFs.map((x:any)=>({name:x.name,key:x.key,group:x.group}))); const src = utmFs.length > 0 ? utmFs : fds; const used = new Set<string>(); const fn = (vs: string[]) => { for (const v of vs) { const m = src.find((x: any) => x.name.toLowerCase().includes(v.toLowerCase()) && !used.has(x.key)); if (m) { used.add(m.key); return m.key } }; return null }; const res = { campaign: fn(['Campanha']), adset: fn(['Conjunto']), creative: fn(['Anuncio', 'Anúncio']) }; await setC(sb, k, res, TTL.s); return res }
