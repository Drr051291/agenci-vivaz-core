...
const getTK = async (sb: any, f: boolean) => { const k = 'tk5'; if (!f) { const c = await cache(sb, k); if (c) return c }; const r = await pd('/v1/dealFields'); const fds = r?.data || []; const grp = fds.filter((x: any) => x.group && typeof x.group === 'object' && (x.group.name || '').toUpperCase().includes('UTM')); const src = grp.length >= 3 ? grp : fds; const used = new Set<string>(); const fn = (vs: string[]) => { for (const v of vs) { const m = src.find((x: any) => x.name.toLowerCase().includes(v.toLowerCase()) && !used.has(x.key)); if (m) { used.add(m.key); return m.key } }; return null }; const res = { campaign: fn(['Campanha']), adset: fn(['Conjunto']), creative: fn(['Anuncio', 'Anúncio']) }; await setC(sb, k, res, TTL.s); return res }
